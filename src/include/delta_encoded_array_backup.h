...
class EliasGammaDeltaEncodedArray : public DeltaEncodedArray<T, sampling_rate> {
 ...
 private:
  ...
  virtual void EncodeDeltas(T *deltas, size_type num_deltas) override {
    uint64_t pos = 0;
  ...
  T BinarySearch (T val, u_int32_t num_elements) {
      // Set the limit on the number of delta values to decode
    pos_type delta_limit = sampling_rate; //Sampling rate
    pos_type samples_idx = -1;
    if (!this->BinarySearchSamples(val, samples_idx, num_elements)) return -1;
    // Get offset into delta bitmap where decoding should start
    uint64_t current_delta_offset = this->delta_offsets_->Get(samples_idx);
    // Adjust the value being searched for
    val -= this->samples_->Get(samples_idx);

    // Initialize the delta index and sum accumulated
    int64_t delta_idx = 0, delta_sum = 0;

    // Fetch delta values
    Bitmap *delta_values = this->deltas_;
    ...
    while (delta_sum < val && delta_idx < delta_limit) {
      ...
      if (block_cnt == 0) {
        ...
        current_delta_offset++;
        uint64_t decoded_value = this->deltas_->GetValPos(current_delta_offset, delta_width)
            + (1ULL << delta_width);
        ...
        while (delta_sum < val && delta_idx < delta_limit) {
          ...
          current_delta_offset++;
          last_decoded_value = this->deltas_->GetValPos(current_delta_offset,
                                                delta_width)
    ...
    uint64_t res = samples_idx * sampling_rate
        + delta_idx;
    ...
    if (val == delta_sum)
      ...
      return -1;        //will change for range queries
...
int BinarySearchSamples (T key, pos_type& sample_offset, u_int64_t num_elements) {
    u_int64_t si = 0;
    u_int64_t ei;
    if (num_elements % sampling_rate == 0) 
        ei = (num_elements / sampling_rate) - 1;
    else 
        ei = (num_elements / sampling_rate);

    u_int64_t m, val;

    while (si <= ei) {
        m = si + ((ei - si) / 2);
        val = this->samples_->Get(m);
        if ( m==ei || (m < ei && val < key && key < this->samples_->Get(m+1))) {    
            sample_offset = m;
            return 1;
        } else if (val > key) {
            si = m + 1;
